  ...
  <div class="ad__card">
    <div class="post__header">
      <div class="post__header-left">
        ...
        <div class="post__date">{{ $news->created_at }}</div>
      </div>
      ...
      <div class="post__actions">
        @auth()
          <div class="post__actions-left">
            ...
            <form id="favourite{{ $news->id }}"
              class="post__pins post__actions-left-item favourite {{ Auth::user()->hasFavouritedNews($news) ? 'active' : '' }}"
              data-id="{{ $news->id }}" action="{{ route('news.favourite.store', $news->id) }}" method="POST">
              @csrf
              <button type="submit" class="post__pins post__actions-left-item">
                ...
                <p class="post__pins_num fav{{ $news->id }}"> {{ $news->favourite->count() }}</p>
              </button>
            </form>
          ...
          <div class="post__actions-right">
            <form id="like{{ $news->id }}"
              class="post__smile post__actions-right-item smile {{ Auth::user()->hasLikedNews($news) ? 'active' : '' }}"
              data-id="{{ $news->id }}" action="{{ route('news.like.store', $news->id) }}" method="POST">
              @csrf
              <button type="submit">
                <svg class="icon" viewBox="0 0 24 24" fill="none" fill="#000F13">
                  <path
              ...
              <p class="post__smile-num like{{ $news->id }}">
                {{ $news->like->count() }}</p>
            </form>

            <form id="dislike{{ $news->id }}"
              class="post__smile-sad post__actions-right-item smile__sad {{ Auth::user()->hasDislikedNews($news) ? 'active' : '' }}"
              data-id="{{ $news->id }}" action="{{ route('news.dislike.store', $news->id) }}" method="POST">
              @csrf
              <button type="submit">
                <svg class="icon" viewBox="0 0 24 24" fill="none" fill="#000F13">
                  <path
              ...
              <p class="post__smile-sad-num dislike{{ $news->id }}">{{ $news->dislike->count() }}</p>
            </form>
          </div>
        @endauth
        @guest
