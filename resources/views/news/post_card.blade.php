           ...
           @auth()
             <div class="post__actions-left">
               ...
               <a class="post__comments post__actions-left-item" href="{{ route('news.show', $new->id) }}">
                 ...
                 <p class="post__coments_num">{{ $new->comments->count() }}</p>
               </a>

               <form id="favourite{{ $new->id }}"
                 class="post__pins post__actions-left-item favourite {{ Auth::user()->hasFavouritedNews($new) ? 'active' : '' }}"
                 data-id="{{ $new->id }}" action="{{ route('news.favourite.store', $new->id) }}" method="POST">
                 @csrf
                 <button type="submit" class="post__pins post__actions-left-item">
                   ...
                   <p class="post__pins_num fav{{ $new->id }}"> {{ $new->favourite->count() }}</p>
                 </button>
               </form>
             </div>
             <div class="post__actions-right">
               <form id="like{{ $new->id }}"
                 class="post__smile post__actions-right-item smile {{ Auth::user()->hasLikedNews($new) ? 'active' : '' }}"
                 data-id="{{ $new->id }}" action="{{ route('news.like.store', $new->id) }}" method="POST">
                 @csrf
                 <button type="submit">
                   <svg class="icon" viewBox="0 0 24 24" fill="none" fill="#000F13">
                     <path
                 ...
                 <p class="post__smile-num like{{ $new->id }}">
                   {{ $new->like->count() }}</p>
               </form>

               <form id="dislike{{ $new->id }}"
                 class="post__smile-sad post__actions-right-item smile__sad {{ Auth::user()->hasDislikedNews($new) ? 'active' : '' }}"
                 data-id="{{ $new->id }}" action="{{ route('news.dislike.store', $new->id) }}" method="POST">
                 @csrf
                 <button type="submit">
                   <svg class="icon" viewBox="0 0 24 24" fill="none" fill="#000F13">
                     <path
                 ...
                 <p class="post__smile-sad-num dislike{{ $new->id }}">{{ $new->dislike->count() }}</p>
               </form>
             </div>
           @endauth
           @guest
             <div class="post__actions-left">
               ...
               <div type="submit" class="post__pins post__actions-left-item">
                 ...
                 <p class="post__pins_num"> {{ $new->favourite->count() }}</p>
               </div>
             ...
             <div class="post__actions-right">
               ...
               <div class="post__smile-sad post__actions-right-item active">
                 ...
                 <p class="post__smile-sad-num">{{ $new->dislike->count() }}</p>
               </div>
             </div>
           @endguest
         </div>
