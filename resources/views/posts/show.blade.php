...
<div class="ad__card">
      ...
      <div class="post__drop">
        <div class="meatballs">
          <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="#000F13">
            <path d="M5 10C3.9 10 3 10.9 3 12C3 13.1 3.9 14 5 14C6.1 14 7 13.1 7 12C7 10.9 6.1 10 5 10Z" stroke-width="1.5"></path>
            <path d="M19 10C17.9 10 17 10.9 17 12C17 13.1 17.9 14 19 14C20.1 14 21 13.1 21 12C21 10.9 20.1 10 19 10Z" stroke-width="1.5"></path>
            <path d="M12 10C10.9 10 10 10.9 10 12C10 13.1 10.9 14 12 14C13.1 14 14 13.1 14 12C14 10.9 13.1 10 12 10Z" stroke-width="1.5"></path>
          </svg>
        </div>
  ...
  <div class="post__main">
    ...
    <div class="post__img">
      <picture>
        <source srcset="{{ asset('storage/' . $post->post_image) }}" type="image/webp"><img src="{{ asset('storage/' . $post->post_image) }}" alt="">
      </picture>
    </div>
    @endif
  ...
  <div class="post__bottom">
    ...
    <div class="post__actions">
      ...
      <div class="post__actions-left">
        ...
        <form class="{{ auth()->user()->favourite ?? 'active' }}" action="{{ route('post.favourite.store', [$post->category_id, $post->id]) }}" method="POST">
          @csrf
      ...
      <div class="post__actions-right">
        ...
        <form class="post__smile post__actions-right-item" action="{{ route('post.like.store', [$post->category_id, $post->id]) }}" method="POST">
          @csrf
          <button type="submit">
            @if (auth()->user()->like)
            <svg class="icon" viewBox="0 0 24 24" fill="none" fill="#000F13">
        ...
        <form class="post__smile-sad post__actions-right-item active" action="{{ route('post.dislike.store', [$post->category_id, $post->id]) }}" method="POST">
          @csrf
          <button type="submit">
...
@auth()
<form class="comment" action="{{ route('post.comment.store', [$post->category_id, $post->id]) }}" method="POST" enctype="multipart/form-data">
  @csrf
  ...
  <div class="comment__bottom">
    ...
    <label for="input__comment"><svg class="icon" viewBox="0 0 24 24" fill="none" stroke="#292D32">
        ...
        <path d="M9 10C10.1046 10 11 9.10457 11 8C11 6.89543 10.1046 6 9 6C7.89543 6 7 6.89543 7 8C7 9.10457 7.89543 10 9 10Z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
        <path d="M2.66998 18.9501L7.59998 15.6401C8.38998 15.1101 9.52998 15.1701 10.24 15.7801L10.57 16.0701C11.35 16.7401 12.61 16.7401 13.39 16.0701L17.55 12.5001C18.33 11.8301 19.59 11.8301 20.37 12.5001L22 13.9001" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
      </svg>
    </label>
    @error('content')
...
<div class="ad__comment">
  <div class="ad__comment-top">
    ...
    <p class="ml-4 user__name">{{ $comment->author->name ?? 'Пользователь не найден' }}</p>
    ...
    <div class="post__drop">
      ...
      <div class="dropdown report">
        ...
        <form action="{{ route('post.comment.destroy', [$post->category_id, $post->id, $comment->id]) }}" method="POST">
          @csrf
          @method('DELETE')
  ...
  <div class="post__img">
    <picture>
      <source srcset="{{ asset('storage/' . $comment->comment_image) }}" type="image/webp"><img src="{{ asset('storage/' . $comment->comment_image) }}" alt="">
    </picture>
  </div>
  @endif
  @auth
...
<div class="ad__comment">
  <div class="ad__comment-top">
    ...
    <div class="post__drop">
      ...
      <div class="dropdown report">
        ...
        <form action="{{ route('post.comment.destroy', [$post->category_id, $post->id, $comment1->id]) }}" method="POST">
          @csrf
          @method('DELETE')
          <button type="submit" class="title__count report" data-micromodal-trigger="report">
            Удалить
  ...
  <div class="post__img">
    <picture>
      <source srcset="{{ asset('storage/' . $comment1->comment_image) }}" type="image/webp"><img src="{{ asset('storage/' . $comment1->comment_image) }}" alt="">
    </picture>
  </div>
  @else
  @endif
