...
@section('content')
        ...
        @auth()
          <div class="post__actions-left">
            ...
            <a class="post__comments post__actions-left-item"
              href="{{ route('post.show', [$post->category_id, $post->slug]) }}">
            ...
            <form id="favourite{{ $post->id }}"
              class="post__pins post__actions-left-item favourite {{ Auth::user()->hasFavouritedPost($post) ? 'active' : '' }}"
              data-id="{{ $post->id }}" action="{{ route('post.favourite.store', [$post->category_id, $post->id]) }}"
              method="POST">
              @csrf
              <button type="submit" class="post__pins post__actions-left-item">
                ...
                <p class="post__pins_num fav{{ $post->id }}"> {{ $post->favourite->count() }}</p>
              </button>
            </form>
          </div>
          <div class="post__actions-right">
            <form id="like{{ $post->id }}"
              class="post__smile post__actions-right-item smile {{ Auth::user()->hasLikedPost($post) ? 'active' : '' }}"
              data-id="{{ $post->id }}" action="{{ route('post.like.store', [$post->category_id, $post->id]) }}"
              method="POST">
              @csrf
              <button type="submit">
                <svg class="icon" viewBox="0 0 24 24" fill="none" fill="#000F13">
              ...
              <p class="post__smile-num like{{ $post->id }}">
                {{ $post->like->count() }}</p>
            </form>

            <form id="dislike{{ $post->id }}"
              class="post__smile-sad post__actions-right-item smile__sad {{ Auth::user()->hasDislikedPost($post) ? 'active' : '' }}"
              data-id="{{ $post->id }}" action="{{ route('post.dislike.store', [$post->category_id, $post->id]) }}"
              method="POST">
              @csrf
              <button type="submit">
                <svg class="icon" viewBox="0 0 24 24" fill="none" fill="#000F13">
              ...
              <p class="post__smile-sad-num dislike{{ $post->id }}">{{ $post->dislike->count() }}</p>
            </form>
          </div>
        @endauth
        @guest
  ...
  @include('posts.comments.comments_post')
  ...
  <script>
    ...
    function ready() {
      ...
      const likeButtonCount = card.querySelector('.smile p')
      const dislikeButton = card.querySelector('.smile__sad')
      const dislikeButtonCount = card.querySelector('.smile__sad p')
