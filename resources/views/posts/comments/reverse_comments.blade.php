    @foreach ($comments_res as $comment)
      <div class="ad__comment">
        <div class="ad__comment-top">
          ...
          <p class="user__name">Ответ пользователю: {{ $comment->parent->author->name ?? 'Пользователь не найден' }}
          </p>
        ...
        @endif
        @auth()
          <form class="comment" action="{{ route('post.comment.store', [$post->category_id, $post->id]) }}"
      ...
      @if (count($comment->replies) > 0)
        @include('posts.comments.reverse_comments', [
