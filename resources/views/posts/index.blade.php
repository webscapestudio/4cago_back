...
@section('content')
  <form class="form__search" action="{{ route('post.search', [$post_cat]) }}" method="get">
    <div class="form__top">
  ...
  <button type="button" id="read-more" class="btn btn__blue read-more" data-cat="{{ $post_cat }}">Показать
    еще</button>

  <script>
    var id_cat = $('#read-more').attr("data-cat");
    var ENDPOINT = "{{ url('/categories') }}";
    var page = 1;
    ...
    function infinteLoadMore(page) {
      $.ajax({
          url: ENDPOINT + "/" + id_cat + "/posts?page=" + page,
          datatype: "html",
          type: "get",
          beforeSend: function() {
            $('.auto-load').show();
    ...
    $(document).ready(function() {
      $('#search').on('keyup', function() {
        ...
        if (value != 0) {
          $.ajax({
            type: "get",
            url: ENDPOINT + "/" + id_cat + "/posts/search",
            data: {
              'search': value
            },
          ...
          $.ajax({
              url: ENDPOINT + "/" + id_cat + "/posts?page=1",
              datatype: "html",
              type: "get",
              beforeSend: function() {
                $('.auto-load').show();
