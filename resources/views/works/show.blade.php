...
@section('content')
  <div class="vacancy__card">
    ...
    <p class="post__title">{{ $work->title }}</p>
    ...
    <div class="vacancy__card-bottom">
      @auth()
        ...
        <form id="favourite{{ $work->id }}"
          ...
          data-id="{{ $work->id }}" action="{{ route('work.favourite.store', [$work->category_work_id, $work->id]) }}"
          method="POST">
          @csrf
