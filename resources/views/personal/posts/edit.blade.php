...
@section('content')
<form class="ad__post" action="{{ route('personal.post.update', $post->slug) }}" method="POST" enctype="multipart/form-data">
  ...
  <div class="post-create__inner">
    ...
    <div class="create__text">
      ...
      <div class="white__area">
        ...
        <textarea class="white__textarea" name="description" id="white-area">{{ $post->description }}</textarea>
        @error('description')
        <div class="text-danger">{{ $message }}</div>
        @enderror
      ...
      @if ($post->post_image)
      <div class="rounded-lg w-[64px] h-[64px] shrink-0 my-4 overflow-hidden border border-gray-300 existedimwrap ">
        <img class=" object-cover block h-full w-full" src="{{ asset('storage/' . $post->post_image) }}" />
      </div>
      @else


      @endif
      <div class="rounded-lg w-[64px] h-[64px] shrink-0 mt-2 overflow-hidden border border-gray-300 loadedimwrap" style="display: none;">
        <img id="output" class=" object-cover block h-full w-full" />
      </div>
      <div class="create__text">
        <p class="post__date">Текст</p>
        <textarea class="tiny_editor" name="content">{{ $post->content }}</textarea>
        @error('content')
        <div class="text-danger">{{ $message }}</div>
        @enderror
      </div>
...
<script>
  var loadFile = function(event) {
    ...
    const existedWrap = document.querySelector('.existedimwrap')
    output.src = URL.createObjectURL(event.target.files[0]);
    output.onload = function() {
      URL.revokeObjectURL(output.src)
      outputWrap.style.display = "block"
      existedWrap.style.display = "none"
    }
  };
</script>
@endsection
