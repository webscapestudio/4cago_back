    ...
    <div class="post-create__inner">
      ...
      <div class="categories">
        <div class="post__category">
          ...
          <form class="custom__select" action="">
      ...
      <div class="create__tags">
        <select class="select2" name="tag_ids[]" multiple="multiple" data-placeholder="Выберите теги" style="width: 100%;">
          @foreach ($tags as $tag)
          <option {{ is_array(old('tag_ids')) && in_array($tag->id, old('tag_ids')) ? ' selected' : '' }} value="{{ $tag->id }}" {{ $tag->id == old('tag_id') ? 'selected' : '' }}>
            {{ $tag->title }}
          </option>
          @endforeach
        </select>
