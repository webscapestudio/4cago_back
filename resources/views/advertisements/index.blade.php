...
@section('content')
  ...
  <script>
    ...
    function ready() {
      const uri = "{{ url('/categories_advertisements') }}"
      let advertisement_cat = "{{ $advertisement_cat }}"
      let page = 1
      ...
      async function searchHandler() {
        ...
        const response = await fetch(`${uri}/${advertisement_cat}/advertisements/search?search=${value}`)
          .then(res => res.text())
          .then(data => {
      ...
      async function fetchPosts(page = 1) {
        preloaderEl.style.display = "flex"
        const response = await fetch(`${uri}/${advertisement_cat}/advertisements?page=${page}`, {
            headers: {
