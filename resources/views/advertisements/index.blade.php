...
@section('content')
  ...
  <button type="button" id="read-more" class="btn btn__blue read-more" data-cat="{{ $advertisement_cat }}">Показать
    еще</button>

  <script>
    var id_cat = $('#read-more').attr("data-cat");
    var ENDPOINT = "{{ url('/categories_advertisements') }}";
    var page = 1;
    ...
    function infinteLoadMore(page) {
      $.ajax({
          url: ENDPOINT + "/" + id_cat + "/advertisements?page=" + page,
          datatype: "html",
          type: "get",
          beforeSend: function() {
            $('.auto-load').show();
    ...
    $(document).ready(function() {
      $('#search').on('keyup', function() {
        ...
        if (value != 0) {
          $.ajax({
            type: "get",
            url: ENDPOINT + "/" + id_cat + "/advertisements/search",
            data: {
              'search': value
            },
          ...
          $.ajax({
              url: ENDPOINT + "/" + id_cat + "/advertisements?page=1",
              datatype: "html",
              type: "get",
              beforeSend: function() {
                $('.auto-load').show();
