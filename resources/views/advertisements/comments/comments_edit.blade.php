...
@section('content')
    <div class="feed">
        ...
        @auth()
            <form class="comment" action="{{ route('advertisement.comment.update', $advertisement->id) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <p class="title__comment">Комментарии</p>
                <textarea id="summernote" class="white__textarea" name="content" id="area__comment">{{ $advertisement->content }}</textarea>
        ...
        @foreach ($advertisement->comments as $comment)
            <div class="ad__comment">
                <div class="ad__comment-top">
                    ...
                    <p class="user__name">{{ $comment->author->name }}</p>
