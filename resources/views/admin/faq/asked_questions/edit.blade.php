          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item"><a href="{{ route('admin.asked_question.index') }}">Правила</a></li>
            ...
            <form class="card card-primary" action="{{ route('admin.asked_question.update', $asked_question->id) }}"
              ...
              <div class="card-body">
                ...
                <select class="form_control" name="published">
                  @if (@isset($asked_question->id))
                    <option value="0" @if ($asked_question->published == 0) selected = "" @endif>Не
                      опубликовано</option>
                    <option value="1" @if ($asked_question->published == 1) selected = "" @endif>
                      Опубликовано</option>
                  @else
                ...
                <select class="form_control" name="category_help_id">
                  @foreach ($categories_help as $category_help)
                    <option value="{{ $category_help->id }}"
                      {{ $category_help->id == $asked_question->categoriy_asked_question->id ? ' selected' : '' }}>
                      {{ $category_help->title }}</option>
                  @endforeach
                </select>
                @error('category_help_id')
