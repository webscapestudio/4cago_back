            ...
            <div class="card card-primary card-outline">
              <div class="card-body box-profile">
                <div class="text-center">
                  @if ($user->user_avatar)
                    <img class="profile-user-img img-fluid img-circle" src="{{ asset('storage/' . $user->user_avatar) }}"
                      alt="User profile picture">
                  @else
                    <img class="img-circle img-bordered-sm" src="{{ asset('/images/avatars/user-car-ava.jpg') }}"
                      alt="User profile picture">
                  @endif
                </div>
                ...
                <ul class="list-group list-group-unbordered mb-3">
                  ...
                  <li class="list-group-item">
                    <b>Обьявлений</b> <a class="float-right">{{ $user->advertisements()->get()->count() }}</a>
                  </li>
                  <li class="list-group-item">
            ...
            <div class="card">
              <div class="card-header p-2">
                <ul class="nav nav-pills">
                  ...
                  <li class="nav-item"><a class="nav-link" href="#vacancy" data-toggle="tab">Все
                      вакансии пользователя</a>
                  </li>
              ...
              <div class="card-body">
                <div class="tab-content">
                      ...
                      @foreach ($user->posts as $post)
                        <div class="post">
                          <div class="user-block">
                            @if ($user->user_avatar)
                              <img class="img-circle img-bordered-sm" src="{{ asset('storage/' . $user->user_avatar) }}"
                                alt="User profile picture">
                            @else
                              <img class="img-circle img-bordered-sm"
                                src="{{ asset('/images/avatars/user-car-ava.jpg') }}" alt="User profile picture">
                            @endif
                            <span class="username">
                              ...
                              <form action="{{ route('admin.post.destroy', $post->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button class="float-right btn btn-danger btn-sm">Удалить</button>
                              </form>
                            </span>
                          ...
                          @if ($post->post_image)
                            <div class="col-sm-6">
                              <img class="img-fluid mb-3" src="{{ asset('storage/' . $post->post_image) }}"
                                alt="User Image">
                            </div>
                          @else
                          @endif
                    ...
                    @if (isset($user->advertisements) and !$user->advertisements->isEmpty())
                      @foreach ($user->advertisements as $advertisement)
                        <div class="post">
                          ...
                          <h4> <a href="{{ route('admin.advertisement.show', $advertisement->id) }}">
                              {{ $advertisement->title }}</a></h4>
                          @if ($advertisement->advertisement_image)
                            <div class="col-sm-6">
                              <img class="img-fluid mb-3"
                                src="{{ asset('storage/' . $advertisement->advertisement_image) }}" alt="User Image">
                            </div>
                          @else
                          @endif
                          <p>
                  ...
                  <div class="tab-pane" id="vacancy">
                    @if (isset($user->works) and !$user->works->isEmpty())
                      @foreach ($user->works as $work)
                        <div class="post">
                          ...
                          <h4> <a href="{{ route('admin.work.show', $work->id) }}">
                              {{ $work->title }}</a></h4>
                          @if ($work->work_image)
                            <div class="col-sm-6">
                              <img class="img-fluid mb-3" src="{{ asset('storage/' . $work->work_image) }}"
                                alt="User Image">
                            </div>
                          @else
                          @endif
                          ...
                          <div class="card-body">
                            <span class="float-right text-muted"> Добавленно в
                              избранное -
